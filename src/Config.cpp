...
Config load_config(const std::string& path)
{
	...
	config.api_enabled = api["enabled"].value_or(true);
	config.api_bind = api["bind"].value_or("127.0.0.1");
	config.api_port = static_cast<uint16_t>(api["port"].value_or(3005));

	config.auto_download = download["auto"].value_or(true);
	config.download_latest_on_first_start = download["latest_on_first_start"].value_or(true);
	config.auto_upload = download["auto_upload"].value_or(true);
	config.max_auto_queue = download["max_auto_queue"].value_or(5);
	config.index_interval_s = std::max(1, download["index_interval"].value_or(30));
	config.upload_interval_s = std::max(1, upload["interval"].value_or(10));
